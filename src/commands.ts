...
function command(session: builder.Session, next: () => void, handoff: Handoff, bot: builder.UniversalBot) {
    ...
    switch (inputWords[0]) {
        ...
        case "list":
            if (inputWords.length === 1) {
                ...
                const conversations = (await handoff.getTeamConversations(team));
                if (!conversations) {
...
async function customerCommand(session: builder.Session, next: () => void, handoff: Handoff, bot: builder.UniversalBot) {
    const message = session.message;
...
async function currentTeams(session: builder.Session, handoff: Handoff): Promise<string[]> {
    const allTeams = await handoff.getCurrentTeams();
